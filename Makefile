...
test: clean .build_history/pylint .build_history/bandit
	...
	# $(VENV) pytest terminaltables --doctest-modules
	$(VENV) python -m unittest discover
    # not all tests run in all OSs, giving seemingly low coverage.
	$(VENV) py.test tests --cov=terminaltables --cov-report=html --cov-fail-under 75
...
.PHONY: mypy
mypy:
	$(VENV) mypy terminaltables --ignore-missing-imports --check-untyped-defs